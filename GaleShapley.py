#Source: https://github.com/amanlai/StableMarriage
import numpy as np
    ...
    def __blocking_pair_among_married_couples(self, Preferences, married, **kwargs):
        ...
        for spouse1, spouse2 in married.items():
            ...
            # if spouse1 is neither proposer nor a receiver, print an error message and end the check
                ...
                raise ValueError(
                    '{} in the matching is on a preference lists of the proposing side when they should be on the receiving side.'.format(
                        spouse1))
    ...
    def Deferred_Acceptance(self, **kwargs):
        ...
        for p, Pref_list in P_m.items():
            ...
            if pointed not in P_w:
                ...
                raise TypeError(
                    "Preference lists must be strict. {} in {}'s preference list implies that {} is indifferent between them.".format(
                        pointed, p, p))
        ...
        while rejections_exist:
            ...
            for p in set(P_m).difference(set(mu)):
                ...
                if recent_proposals[p] != P_m[p][-1]:
                    ...
                    if pointed not in P_w:
                        raise ValueError(
                            "Proposer {}'s preference list includes {}, who is not present in this problem."
                            .format(p, pointed))

                    elif isinstance(pointed, list):
                        raise ValueError(
                            "The preference lists must be strict. {} in {}'s preference list implies {} is indifferent between them.".format(
                                pointed, p, p))
